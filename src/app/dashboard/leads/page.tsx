      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <div className="text-red-600 mb-4">
            {error instanceof Error ? error.message : "فشل في تحميل العملاء المحتملين"}
          </div>
          <button
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                title="تحديث"
              >
                <ArrowPathIcon className={cn("h-5 w-5", isRefetching && "animate-spin")} />
              </button>
            </div>
          </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-600 leading-relaxed">
                  العملاء المحتملون المتاحون يظهرون بعلامة <span className="inline-flex items-center mx-1 px-2 py-0.5 rounded-full bg-red-500 border text-white border-gray-200 text-[8px] font-semibold"><span className="w-1 h-1 bg-white rounded-full ml-1"></span>متاح الآن</span> ويمكن لأي موظف مبيعات استلامها.
                  بمجرد الاستلام، يتم تخصيص العميل لك تلقائياً ولا يمكن لأحد آخر الوصول إليه.
                </p>
              </div>
            </div>
          </div>
      ...
      >
        {leadsData.leads.length > 0 && (
          <div className="relative bg-white px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-gray-200">
            <nav className="flex items-center gap-2" aria-label="Pagination" dir="ltr">
              <button
      ...
      {leadsData.leads.length > 0 && (
        <div className="bg-white px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-gray-200">
          <p className="text-sm text-gray-500">
            عرض <span className="font-medium text-gray-900">{(page - 1) * limit + 1}</span>{" "}
            إلى <span className="font-medium text-gray-900">{Math.min(page * limit, leadsData.total)}</span>{" "}
            من <span className="font-medium text-gray-900">{leadsData.total}</span> نتائج
          </p>
        </div>
      )}
    </div>
